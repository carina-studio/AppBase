...
abstract class BasePackageResolverTests: BaseTests
{
	...
	public void CancellationTest()
	{
		this.TestOnApplicationThread(async () =>
		{
			...
			var informationalVersion = $"{DateTime.Now.Year}.2.3.4 Test";
			...
			var packageManifest = this.GeneratePackageManifest(appName, version, informationalVersion, pageUri, packageInfos);
			...
			using (var packageResolver = this.CreateInstance(packageManifest))
			{
				...
				await packageResolver.WaitForPropertyChangeAsync(nameof(IUpdaterComponent.State), r => r.State == UpdaterComponentState.Cancelled, cancellationTokenSource.Token);
				Assert.That(packageResolver.State == UpdaterComponentState.Cancelled);
			}
			...
			using (var packageResolver = this.CreateInstance(packageManifest))
			{
				...
				cancellationTokenSource.CancelAfter(10000);
				Assert.That(packageResolver.Start());
				await packageResolver.WaitForPropertyChangeAsync(nameof(IUpdaterComponent.State), r => r.State == UpdaterComponentState.Succeeded, cancellationTokenSource.Token);
				Assert.That(!packageResolver.Cancel());
			...
			for (var t = 0; t < 100; ++t)
			{
				...
				if (packageResolver.Cancel())
				{
					Assert.That(UpdaterComponentState.Cancelling == packageResolver.State);
					await packageResolver.WaitForPropertyChangeAsync(nameof(IUpdaterComponent.State), r => r.State == UpdaterComponentState.Cancelled, cancellationTokenSource.Token);
					Assert.That(UpdaterComponentState.Cancelled == packageResolver.State);
				}
				else
				{
					Assert.That(UpdaterComponentState.Cancelling != packageResolver.State);
	...
	/// <param name="informationalVersion">Application informational version.</param>
	/// <param name="pageUri">URI of package web page.</param>
	/// <param name="packageInfos">Package info list.</param>
	/// <returns></returns>
	protected abstract string GeneratePackageManifest(string? appName, Version? version, string? informationalVersion, Uri? pageUri, IList<PackageInfo> packageInfos);
	...
	public void ResolvingTest()
	{
		this.TestOnApplicationThread(async () =>
		{
			...
			using (var packageResolver = this.CreateInstance(packageManifest))
			{
				...
				Assert.That(UpdaterComponentState.Succeeded == packageResolver.State);
				this.VerifyResolvedPackage(packageResolver, appName, version, informationalVersion, pageUri, expectedPackageUri, expectedMD5, expectedSHA256, expectedSHA512);
			}
			...
			packageManifest = this.GeneratePackageManifest(null, version, informationalVersion, pageUri, packageInfos);
			using (var packageResolver = this.CreateInstance(packageManifest))
			{
				using var cancellationTokenSource = new CancellationTokenSource();
				cancellationTokenSource.CancelAfter(10000);
				...
				Assert.That(packageResolver.Exception is null);
				Assert.That(UpdaterComponentState.Succeeded == packageResolver.State);
				this.VerifyResolvedPackage(packageResolver, null, version, informationalVersion, pageUri, expectedPackageUri, expectedMD5, expectedSHA256, expectedSHA512);
			}

			// resolve package info without version
			packageManifest = this.GeneratePackageManifest(appName, null, informationalVersion, pageUri, packageInfos);
			using (var packageResolver = this.CreateInstance(packageManifest))
			{
				using var cancellationTokenSource = new CancellationTokenSource();
				cancellationTokenSource.CancelAfter(10000);
				...
				Assert.That(packageResolver.Exception is null);
				Assert.That(UpdaterComponentState.Succeeded == packageResolver.State);
				this.VerifyResolvedPackage(packageResolver, appName, null, informationalVersion, pageUri, expectedPackageUri, expectedMD5, expectedSHA256, expectedSHA512);
			}
			
			// resolve package info without informational version
			packageManifest = this.GeneratePackageManifest(appName, version, null, pageUri, packageInfos);
			using (var packageResolver = this.CreateInstance(packageManifest))
			{
				using var cancellationTokenSource = new CancellationTokenSource();
				cancellationTokenSource.CancelAfter(10000);
				...
				Assert.That(packageResolver.Exception is null);
				Assert.That(UpdaterComponentState.Succeeded == packageResolver.State);
				this.VerifyResolvedPackage(packageResolver, appName, version, null, pageUri, expectedPackageUri, expectedMD5, expectedSHA256, expectedSHA512);
			}

			// resolve package info without package list
			packageManifest = this.GeneratePackageManifest(appName, version, informationalVersion, pageUri, Array.Empty<PackageInfo>());
			using (var packageResolver = this.CreateInstance(packageManifest))
			{
				using var cancellationTokenSource = new CancellationTokenSource();
				cancellationTokenSource.CancelAfter(10000);
				Assert.That(packageResolver.Start());
				await packageResolver.WaitForPropertyChangeAsync(nameof(IUpdaterComponent.State), r => r.State == UpdaterComponentState.Failed, cancellationTokenSource.Token);
				Assert.That(UpdaterComponentState.Failed == packageResolver.State);
			}
			...
			if (Platform.IsNotMacOS)
			{
				...
				packageManifest = this.GeneratePackageManifest(appName, version, informationalVersion, pageUri, packageInfos);
	...
	void VerifyResolvedPackage(IPackageResolver packageResolver, string? appName, Version? version, string? informationalVersion, Uri? pageUri, Uri? packageUri, string? md5 = null, string? sha256 = null, string? sha512 = null)
	{
		Assert.That(appName == packageResolver.ApplicationName);
		Assert.That(version == packageResolver.PackageVersion);
		Assert.That(informationalVersion == packageResolver.PackageInformationalVersion);
