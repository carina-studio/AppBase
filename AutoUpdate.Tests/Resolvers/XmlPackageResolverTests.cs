...
class XmlPackageResolverTests : BasePackageResolverTests
{
	...
	protected override string GeneratePackageManifest(string? appName, Version? version, string? informationalVersion, Uri? pageUri, IList<PackageInfo> packageInfos)
	{
		...
		informationalVersion?.Let(it => xmlWriter.WriteAttributeString("InformationalVersion", it));
